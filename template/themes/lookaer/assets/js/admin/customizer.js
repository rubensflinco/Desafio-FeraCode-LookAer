...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.lookaer webkitAnimationEnd.lookaer', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.lookaer webkitAnimationEnd.lookaer' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.lookaer webkitAnimationEnd.lookaer', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.lookaer webkitAnimationEnd.lookaer' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.lookaer', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.lookaer' );
			} );
		},
